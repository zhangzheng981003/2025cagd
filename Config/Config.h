...
PRJ_BEGIN
//extern std::vector<int> iscollision;
...
extern int res;
extern int is_good_init;
extern int iter;
extern double stop_condition,data_energy0 ;
extern double data_e;
extern int insert_idx,iter_,zero_num;
...
extern int subdivision_number;
extern double barrier_lambda;
...
//@brief 0927 start.
extern double data_weight, smooth_weight, barrier_weight, fast_barrier_weight;

extern std::vector<std::vector<Eigen::VectorXd>> divide_ctrpoint_;
extern std::vector<std::vector<std::vector<double>>> divide_u_knot_;
extern std::vector<std::vector<Eigen::MatrixXd>> divide_basis_;
...
class Conversion
{
  ...
  */
  ...
  static void gcl_convert_matrix()
  {
      Eigen::MatrixXd M(N + 1, N + 1);
      M.setIdentity();
      convert_list.resize(piece_num);
