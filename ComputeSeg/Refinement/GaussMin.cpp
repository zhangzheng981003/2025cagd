...
{
//	int n_he = mesh_.n_halfedges();
//	Eigen::MatrixX3d norm_he(n_he, 3);
//#pragma omp parallel for
//	for (int i = 0; i < n_he; i++)
//	{
//		int to_idx = mesh_.to_vertex_handle(HEH(i)).idx();
//		int from_idx = mesh_.from_vertex_handle(HEH(i)).idx();
//
//		norm_he.row(i) = new_pos_.row(to_idx) - new_pos_.row(from_idx);
//	}
//
//	norm_he.rowwise().normalize();
//	
//	norm_K_.fill(M_PI * 2);
...
//	for (int i = 0; i < n_f; ++i)
//	{
//		HEH cur_he = mesh_.halfedge_handle(FH(i));
//		HEH next_he = mesh_.next_halfedge_handle(cur_he);
//		HEH prev_he = mesh_.next_halfedge_handle(next_he);
//
//		int cur_idx = mesh_.opposite_vh(cur_he).idx();
//		int next_idx = mesh_.opposite_vh(next_he).idx();
//		int prev_idx = mesh_.opposite_vh(prev_he).idx();
//
//		if (glo2inner_[cur_idx] != -1)
//		{
//			norm_K_(glo2inner_[cur_idx]) -= M_PI
//		- acos(norm_he.row(next_he.idx()).dot(norm_he.row(prev_he.idx())));
//		}
//
//		if (glo2inner_[next_idx] != -1)
//		{
//			norm_K_(glo2inner_[next_idx]) -= M_PI
//				- acos(norm_he.row(prev_he.idx()).dot(norm_he.row(cur_he.idx())));
//		}
//
//		if (glo2inner_[prev_idx] != -1)
//		{
//			norm_K_(glo2inner_[prev_idx]) -= M_PI
//				- acos(norm_he.row(cur_he.idx()).dot(norm_he.row(next_he.idx())));
//		}
//	}
//
//	norm_K_ /= cur_bound_;
...
void GaussMin::ComputeR()
{
	...
	//	(new_pos_ - ori_pos_).rowwise().squaredNorm().array() - m_dist_ * m_dist_)).matrix();
...
#pragma omp parallel for num_threads(6)
	for (int i = 0; i < 6; i++)
	{
		...
		K_triplet[thread_num] = ComputeTempTriplet(start_idx, thread_num);
	}
	...
	//// exp close
	//Eigen::MatrixX3d temp_matrix = lambda_exp_close_ * (new_pos_ - ori_pos_);
	//std::vector<T> exp_close_triplet;
	//for (size_t i = 0; i < n_v; i++)
	//{
	//	exp_close_triplet.push_back(T(start_idx + i, i, temp_matrix(i, 0) * r_exp_close_(i)));
	//	exp_close_triplet.push_back(T(start_idx + i, i + n_v, temp_matrix(i, 1) * r_exp_close_(i)));
	//	exp_close_triplet.push_back(T(start_idx + i, i + 2 * n_v, temp_matrix(i, 2) * r_exp_close_(i)));
	//}
	//start_idx += n_v;

	// exp close
	Eigen::MatrixX3d temp_matrix = lambda_exp_close_ * (new_pos_ - ori_pos_) / m_dist_ / m_dist_;
	std::vector<T> exp_close_triplet;
	for (size_t i = 0; i < n_v; i++)
	{
		exp_close_triplet.push_back(T(start_idx + i, i, temp_matrix(i, 0) * r_exp_close_(i)));
	...
	// exp smooth
	temp_matrix = lambda_exp_smooth_ * m_sm_.transpose() * m_sm_ * new_pos_;
	std::vector<T> exp_smooth_triplet;
	for (size_t i = 0; i < n_inner; i++)
	{
		exp_smooth_triplet.push_back(T(start_idx + i, i, temp_matrix(i, 0) * r_exp_smooth_(i)));
		exp_smooth_triplet.push_back(T(start_idx + i, i + n_v, temp_matrix(i, 1) * r_exp_smooth_(i)));
		exp_smooth_triplet.push_back(T(start_idx + i, i + 2 * n_v, temp_matrix(i, 2) * r_exp_smooth_(i)));
	}
	start_idx += n_inner;

	// exp seam
	temp_matrix = lambda_exp_seam_ * m_se_.transpose() * m_se_ * new_pos_;
	std::vector<T> exp_seam_triplet;
	for (size_t i = 0; i < n_seam; i++)
	{
		exp_seam_triplet.push_back(T(start_idx + i, i, temp_matrix(i, 0) * r_exp_seam_(i)));
		exp_seam_triplet.push_back(T(start_idx + i, i + n_v, temp_matrix(i, 1) * r_exp_seam_(i)));
		exp_seam_triplet.push_back(T(start_idx + i, i + 2 * n_v, temp_matrix(i, 2) * r_exp_seam_(i)));
	}
	start_idx += n_seam;
...
//void GaussMin::SetInitJTJ()
//{
//	int n_v = mesh_.n_vertices();
//
//	SpMat m_sum = lambda_close_ * lambda_close_ * m_id_.transpose() * m_id_
//		+ lambda_smooth_ * lambda_smooth_ * m_sm_.transpose() * m_sm_
//		+ lambda_seam_ * lambda_seam_ * m_se_.transpose() * m_se_;
//
//	std::vector<T> triplet;
//	triplet.reserve(3 * m_sum.nonZeros());
//	for (int k = 0; k < m_sum.outerSize(); ++k)
//	{
//		for (SpMat::InnerIterator it(m_sum, k); it; ++it)
//		{
//			triplet.push_back(T(it.row(), it.col(), it.value()));
//			triplet.push_back(T(n_v + it.row(), n_v + it.col(), it.value()));
//			triplet.push_back(T(2 * n_v + it.row(), 2 * n_v + it.col(), it.value()));
//		}
//	}
//
//	m_init_JTJ_.resize(3 * n_v, 3 * n_v);
//	m_init_JTJ_.setFromTriplets(triplet.begin(), triplet.end());
//}
...
//	}
//
//	partial_E_.resize(inner2glo_.size(), 3 * mesh_.n_vertices());
//	partial_E_.setFromTriplets(triplet.begin(), triplet.end());
//}

//void GaussMin::ComputeJTJ()
//{
//	m_JTJ_ = m_init_JTJ_ + partial_E_.transpose() * partial_E_ * lambda_dev_ * lambda_dev_;
//}
//
//void GaussMin::ComputeJTd()
//{
//	const int& n_v = mesh_.n_vertices();
//	
//	v_JTd_ = partial_E_.transpose() * Eigen::pow(norm_K_.array(), p_).matrix() * lambda_dev_ * lambda_dev_;
//
//	Eigen::MatrixX3d m_temp_sum 
//		= m_id_.transpose() * (m_id_ * (new_pos_ - ori_pos_)) * lambda_close_ * lambda_close_
//		+ m_sm_.transpose() * (m_sm_ * new_pos_) * lambda_smooth_ * lambda_smooth_
//		+ m_se_.transpose() * (m_se_ * new_pos_) * lambda_seam_ * lambda_seam_;
//
//	v_JTd_.segment(0, n_v) += m_temp_sum.col(0);
//	v_JTd_.segment(n_v, n_v) += m_temp_sum.col(1);
//	v_JTd_.segment(2 * n_v, n_v) += m_temp_sum.col(2);
//}
