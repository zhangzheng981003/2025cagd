...
namespace aabb
{
    ...
    bool AABB::overlaps(const AABB& aabb, const AABB& aabb1, bool touchIsOverlap, double margin) const
    {
        assert(aabb.lowerBound.size() == aabb1.lowerBound.size());

        bool rv = true;

        if (touchIsOverlap)
        {
            for (unsigned int i = 0; i < aabb1.lowerBound.size(); ++i)
            {

                if (aabb.upperBound[i] + margin < aabb1.lowerBound[i] || aabb.lowerBound[i] > aabb1.upperBound[i] + margin)
                {
    ...
    std::vector<unsigned int> Tree::query(unsigned int particle, const AABB& aabb, double margin)
    {
        ...
        while (stack.size() > 0)
        {
            ...
            if (aabb.overlaps(nodeAABB, aabb, touchIsOverlap, margin))
            {
    ...
    std::vector<std::pair<unsigned int, unsigned int>> Tree::query(const Tree& _tree, double margin)
    {
        ...
        while (stack.size() > 0)
        {
            ...
            */
            ...
            // Test for overlap between the AABBs.
            if (_tree.nodes[_node].aabb.overlaps(nodeAABB, _tree.nodes[_node].aabb, touchIsOverlap, margin))
            {
                // Check that we're at a leaf node.
