...
namespace LoopGen
{
	...
	{ 
		...
		HalfedgeLayer* hl; 
		VertexLayer() {} 
		VertexLayer(Mesh::VertexHandle v_, /*int layer_, */int id_) : v(v_), /*layer(layer_), */id(id_), hl(nullptr) {}
	};
	struct FaceLayer
	{
		Mesh::FaceHandle f;
		...
		HalfedgeLayer* hl; 
		FaceLayer() {}
		FaceLayer(Mesh::FaceHandle f_, /*int layer_, */int id_) :f(f_), /*layer(layer_), */id(id_), hl(nullptr) {}
	};
	...
	public:
		...
		int calc_shift(Mesh::VertexHandle v, HalfedgeHandle h)
		{
		...
		void update()
		{
			...
			for (auto tf : mesh->faces())
			{
				...
				std::vector<Mesh::VertexHandle> vhs(3);
			...
			{
				Mesh::VertexHandle v0 = te.v0();
				Mesh::VertexHandle v1 = te.v1();
