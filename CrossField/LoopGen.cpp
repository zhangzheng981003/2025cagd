...
#pragma endregion
	...
	bool LoopGen::BFS(int sid, std::vector<Mesh::VertexHandle> &path, BoolVector &constraint_info, BoolVector &break_info)
	{
		int nv = mesh->n_vertices();
		...
		Mesh::VertexHandle prev_v = mesh->from_vertex_handle(prev[fromid]);
	...
	PlaneLoop& LoopGen::better_cutter(std::vector<Mesh::VertexHandle> &path, PLS &field_path)
	{
