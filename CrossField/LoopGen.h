...
namespace LoopGen
{
	...
	//private:
		...
		//基础函数
		void SetModelName(std::string& name) { model_name = name;  }
		...
		//划分区域阶段
		bool BFS(int sid, std::vector<Mesh::VertexHandle> &path, BoolVector &constraint_info, BoolVector &break_info);
		bool FieldAlignedPath(VertexLayer* vl, PlaneLoop &path, BoolVector &break_info);
		void DivideRegion();
		void ExtractRegion();

		std::vector<std::vector<Mesh::VertexHandle>> sing_connector;
		PlaneLoop& better_cutter(std::vector<Mesh::VertexHandle> &path, PLS &field_path);
		PLS sing_cutter;
