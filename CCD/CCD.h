...
class CCD 
{
  public:
    ...
    static bool gcl_GJKCCD(Data position, Data direction, Data _position, double d, double tMin, double tMax)
    {
        ...
        for (int i = 0; i < position.rows(); i++)
        {
            vrtx1[i + position.rows()] = (double*)malloc(3 * sizeof(double));

            for (int j = 0; j < 3; j++)
            {
                vrtx1[i + position.rows()][j] = position1(i, j);
            }
        }
        ...
        std::cout << "gjk_dist" << gjk_distance2 << std::endl;
        // Print distance between objects. 
        //printf ("Distance between bodies %i\n", intersection);
        return gjk_distance2 <= d * d;
    }

    static bool gcl_KDOPCCD(Data position, Data direction, Data _position, double d, double tMin, double tMax)
    {
        Data A(2 * position.rows(), 3);
        ...
        for (int k = 0; k < dim; k++)
        {
            ...
            {
                //if(k==dim-1) std::cout << upperB << " " << lowerB << " " << upperA << " " << lowerA << std::endl;
            }
        }
    ...
    static bool gcl_KDOPDCD(Data position, Data _position, double d)
    {
        Data A(position.rows(), 3);
        A << position;
